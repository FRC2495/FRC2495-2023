...
public class Rotator extends SubsystemBase implements IRotator {
	...
	public Rotator(WPI_TalonSRX rotator_in, /*BaseMotorController rotator_follower_in,*/ Robot robot_in) {
		...
		//rotator_follower.configFactoryDefault();
		
		// Mode of operation during Neutral output may be set by using the setNeutralMode() function.
		// As of right now, there are two options when setting the neutral mode of a motor controller,
		// brake and coast.
		rotator.setNeutralMode(NeutralMode.Brake);
		//rotator_follower.setNeutralMode(NeutralMode.Brake);
		...
		rotator.configReverseLimitSwitchSource(LimitSwitchSource.FeedbackConnector, LimitSwitchNormal.NormallyOpen, TALON_TIMEOUT_MS);
		rotator.overrideLimitSwitchesEnable(true);
		...
		//rotator_follower.follow(rotator);
		...
		//rotator_follower.setStatusFramePeriod(StatusFrame.Status_1_General, 255, TALON_TIMEOUT_MS);
	...
	public boolean tripleCheckMove() {
		if (isMoving) {
			...
			double error = rotator.getClosedLoopError(PRIMARY_PID_LOOP);
			...
			if (isOnTarget) { // if we are on target in this iteration 
				...
				if (onTargetCount > 0) { // even though we were on target at least once during a previous iteration
					...
					System.out.println("Triple-check failed (rotator moving).");
				} else {
			...
			if (!isMoving) {
				System.out.println("You have reached the target (rotator moving).");
	...
	public int getEncoderVelocity() {
		return (int) (rotator.getSelectedSensorVelocity(PRIMARY_PID_LOOP));
	}
	...
	public double getEncoderPosition() {
		return rotator.getSelectedSensorPosition(PRIMARY_PID_LOOP);
	}
	...
	private void setPIDParameters() {		
		rotator.configAllowableClosedloopError(SLOT_0, TALON_TICK_THRESH, TALON_TIMEOUT_MS);
		...
		rotator.config_kP(SLOT_0, MOVE_PROPORTIONAL_GAIN, TALON_TIMEOUT_MS);
		rotator.config_kI(SLOT_0, MOVE_INTEGRAL_GAIN, TALON_TIMEOUT_MS);
		rotator.config_kD(SLOT_0, MOVE_DERIVATIVE_GAIN, TALON_TIMEOUT_MS);
		rotator.config_kF(SLOT_0, 0, TALON_TIMEOUT_MS);
	}
	
	// NOTE THAT THIS METHOD WILL IMPACT BOTH OPEN AND CLOSED LOOP MODES
	public void setNominalAndPeakOutputs(double peakOutput)
	{
		rotator.configPeakOutputForward(peakOutput, TALON_TIMEOUT_MS);
		rotator.configPeakOutputReverse(-peakOutput, TALON_TIMEOUT_MS);
		
		rotator.configNominalOutputForward(0, TALON_TIMEOUT_MS);
		rotator.configNominalOutputReverse(0, TALON_TIMEOUT_MS);
	}
	...
	public void resetEncoder() {
		rotator.set(ControlMode.PercentOutput,0); // we stop AND MAKE SURE WE DO NOT MOVE WHEN SETTING POSITION
		rotator.setSelectedSensorPosition(0, PRIMARY_PID_LOOP, TALON_TIMEOUT_MS); // we mark the virtual zero
	}
