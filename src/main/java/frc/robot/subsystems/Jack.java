...
/**
 * The {@code Shoulder} class contains fields and methods pertaining to the function of the jack.
 */
public class Jack extends SubsystemBase implements IJack {
	...
	*/
	...
	WPI_TalonSRX jack;
	//BaseMotorController jack_follower;
	...
	public Jack(WPI_TalonSRX jack_in/*, BaseMotorController jack_follower_in*/, Robot robot_in) {
		jack = jack_in;
		//jack_follower = jack_follower_in;
		robot = robot_in;
		
		jack.configFactoryDefault();
		//jack_follower.configFactoryDefault();

		// Mode of operation during Neutral output may be set by using the setNeutralMode() function.
		// As of right now, there are two options when setting the neutral mode of a motor controller,
		// brake and coast.	
		jack.setNeutralMode(NeutralMode.Brake);
		//jack_follower.setNeutralMode(NeutralMode.Brake);
		
		// Sensor phase is the term used to explain sensor direction.
		// In order for limit switches and closed-loop features to function properly the sensor and motor has to be in-phase.
		// This means that the sensor position must move in a positive direction as the motor controller drives positive output.
		jack.setSensorPhase(true);

		// Enables limit switches
		jack.configForwardLimitSwitchSource(LimitSwitchSource.FeedbackConnector, LimitSwitchNormal.NormallyOpen, TALON_TIMEOUT_MS);
		jack.configReverseLimitSwitchSource(LimitSwitchSource.FeedbackConnector, LimitSwitchNormal.NormallyOpen, TALON_TIMEOUT_MS);
		jack.overrideLimitSwitchesEnable(true);

		// Motor controller output direction can be set by calling the setInverted() function as seen below.
		// Note: Regardless of invert value, the LEDs will blink green when positive output is requested (by robot code or firmware closed loop).
		// Only the motor leads are inverted. This feature ensures that sensor phase and limit switches will properly match the LED pattern
		// (when LEDs are green => forward limit switch and soft limits are being checked). 	
		jack.setInverted(true); // invert if required
		//jack_follower.setInverted(false);

		// Both the Talon SRX and Victor SPX have a follower feature that allows the motor controllers to mimic another motor controller's output.
		// Users will still need to set the motor controller's direction, and neutral mode.
		// The method follow() allows users to create a motor controller follower of not only the same model, but also other models
		// , talon to talon, victor to victor, talon to victor, and victor to talon.
		//jack_follower.follow(jack);
		...
		//jackfollower.setStatusFramePeriod(StatusFrame.Status_1_General, 255, TALON_TIMEOUT_MS);
		//jackFollower.setStatusFramePeriod(StatusFrame.Status_2_Feedback0, 255, TALON_TIMEOUT_MS);

		setPIDParameters();
		
		// use slot 0 for closed-looping
 		//jack.selectProfileSlot(SLOT_0, PRIMARY_PID_LOOP);
	...
	public boolean tripleCheckMove() {
		if (isMoving) {
			...
			double error = jack.getClosedLoopError(PRIMARY_PID_LOOP);
			...
			if (isOnTarget) { // if we are on target in this iteration 
				...
				if (onTargetCount > 0) { // even though we were on target at least once during a previous iteration
					...
					System.out.println("Triple-check failed (jack moving).");
				} else {
			...
			if (!isMoving) {
				System.out.println("You have reached the target (jack moving).");
	...
	public int getEncoderVelocity() {
		return (int) (jack.getSelectedSensorVelocity(PRIMARY_PID_LOOP));
	}
	...
	public double getPosition() {
		return jack.getSelectedSensorPosition(PRIMARY_PID_LOOP) * GEAR_RATIO / TICKS_PER_REVOLUTION;
	}

	public double getEncoderPosition() {
		return jack.getSelectedSensorPosition(PRIMARY_PID_LOOP);
	}
	...
	private void setPIDParameters() {		
		jack.configAllowableClosedloopError(SLOT_0, TALON_TICK_THRESH, TALON_TIMEOUT_MS);
		...
		jack.config_kP(SLOT_0, MOVE_PROPORTIONAL_GAIN, TALON_TIMEOUT_MS);
		jack.config_kI(SLOT_0, MOVE_INTEGRAL_GAIN, TALON_TIMEOUT_MS);
		jack.config_kD(SLOT_0, MOVE_DERIVATIVE_GAIN, TALON_TIMEOUT_MS);
		jack.config_kF(SLOT_0, 0, TALON_TIMEOUT_MS);
	}

	public void setNominalAndPeakOutputs(double peakOutput)
	{
		jack.configPeakOutputForward(peakOutput, TALON_TIMEOUT_MS);
		jack.configPeakOutputReverse(-peakOutput, TALON_TIMEOUT_MS);
		
		jack.configNominalOutputForward(0, TALON_TIMEOUT_MS);
		jack.configNominalOutputForward(0, TALON_TIMEOUT_MS);
	}
	...
	public void joystickControl(Joystick joystick)
	{
		...
		{
			jack.set(ControlMode.PercentOutput, -joystick.getY());
		}
	}	
	...
	public boolean getLimitSwitchState() {
		return jack.getSensorCollection().isRevLimitSwitchClosed();
	}

	public boolean getForwardLimitSwitchState() {
		return jack.getSensorCollection().isFwdLimitSwitchClosed();
	}

	// MAKE SURE THAT YOU ARE NOT IN A CLOSED LOOP CONTROL MODE BEFORE CALLING THIS METHOD.
	// OTHERWISE THIS IS EQUIVALENT TO MOVING TO THE DISTANCE TO THE CURRENT ZERO IN REVERSE! 
	public void resetEncoder() {
		jack.set(ControlMode.PercentOutput,0); // we stop AND MAKE SURE WE DO NOT MOVE WHEN SETTING POSITION
		jack.setSelectedSensorPosition(0, PRIMARY_PID_LOOP, TALON_TIMEOUT_MS); // we mark the virtual zero
	}
