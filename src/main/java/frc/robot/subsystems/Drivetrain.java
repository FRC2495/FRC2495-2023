...
public class Drivetrain extends SubsystemBase implements /*PIDOutput, PIDOutput2, PIDOutput3,*/ IDrivetrain {
	...
	public static final double MIN_TURN_USING_CAMERA_PCT_OUTPUT = 0.15; // 0.4;
	...
	static final double MIN_TURN_PCT_OUTPUT = 0.2; // 0.3;
	...
	// joystick control
	{
		if (!isMoving && !isTurning && !isMovingUsingCamera && !isTurningUsingCamera) // if we are already doing a move or turn we don't take over
		{
			...
			//{
				...
				differentialDrive.arcadeDrive(-joyRight.getX() * 0.75, -joyLeft.getY()); // right needs to be reversed
			//}
		}
	}	
